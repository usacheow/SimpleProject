...
class SmsCodeViewModel @Inject constructor(
    ...
    private val _updateStateAction = Channel<SmsCodeState>()
    val updateStateAction = _updateStateAction.receiveAsFlow()
    ...
    fun onResendClicked() = viewModelScope.launch {
        timerJob = startTimer()
        _updateStateAction.send(SmsCodeState.CodeRequested)
    }
    ...
    private fun onCodeInputted() = viewModelScope.launch {
        _updateStateAction.send(SmsCodeState.CodeInputted(_inputtedCodeState.value))
    }

    fun onCodeSucceed() = viewModelScope.launch {
        _updateStateAction.send(SmsCodeState.CodeSucceed)
    }

    fun onCodeFailed(message: String) = viewModelScope.launch {
        _updateStateAction.send(SmsCodeState.CodeFailed(message))
    }
}
...
sealed class SmsCodeState {
    ...
    data class CodeFailed(val message: String) : SmsCodeState()

    object CodeSucceed : SmsCodeState()
