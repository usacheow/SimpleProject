...
import com.usacheow.appstate.otp.OtpResultState
...
class SmsCodeModalFragment : SimpleModalFragment<FragmentSmsCodeBinding>() {
    ...
    override fun subscribe() {
        ...
        viewModel.updateStateAction.observe(lifecycle) {
            when (it) {
                is SmsCodeState.CodeInputted -> {
                    binding.loaderView.root.makeVisible()
                    otpStateViewModel.onCodeInputted(it.code)
                }
                is SmsCodeState.CodeSucceed -> {
                    binding.loaderView.root.makeGone()
                    dismiss()
                }
                is SmsCodeState.CodeFailed -> {
                    binding.loaderView.root.makeGone()
                    binding.smsCodeMessageView.text = it.message
                }
        ...
        otpStateViewModel.updateCodeResultStateAction.observe(lifecycle) {
            when (it) {
                is OtpResultState.CodeSucceed -> viewModel.onCodeSucceed()
                is OtpResultState.CodeFailed -> viewModel.onCodeFailed(it.message)
            }
        }
    }
